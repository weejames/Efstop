...
class S3 {
	...
	var $bucketname = "images.getefstop.net";
	...
	*/
	function S3($options = NULL) {
		...
		if (!defined('DATE_RFC822_S3')) define('DATE_RFC822_S3', 'D, d M Y H:i:s T');
	...
	* Gets the content length of the object
	*/
	...
	*/
	...
	function getPrivateURL($bucket, $object)
	{
	  ...
	  return "http://".$bucket.".".$this->S3_URL."$object?AWSAccessKeyId=".$this->keyId."&Expires=$expires&Signature=$sig";
	}
	...
	*/
	function sendRequest ($resource, $verb = NULL, $objectdata = NULL, $acl = NULL, $contentType = NULL, $metadata = NULL, $bucket = NULL) {
		...
		$this->debug_text("HTTP Request sent to: http://". $bucket . "." . $this->S3_URL . $resource . $paramstring);
		
		$req =& new HTTP_Request("http://". $bucket . "." . $this->S3_URL . $resource . $paramstring);
		$req->setMethod($verb);
		...
		$stringToSign = "$verb\n$contentmd5string\n$contenttypestring\n$httpDate\n$aclstring$metadatastring/$bucket/$resource";
		...
		$this->_responseContentLength = $req->getResponseHeader("content-length");
	...
	*/
	function getObjectInfo ($objectname, $bucketname = NULL) {
		...
		return $this->sendRequest($objectname,"HEAD", null, null, null, null, $bucketname);
	}
	...
	*/
	function getObject ($objectname, $bucketname = NULL) {
		...
		return $this->sendRequest($objectname,"GET", null, null, null, null, $bucketname);
	}
	...
	*/
	function putObject ($objectname, $objectdata, $bucketname = NULL, $acl = NULL, $contentType = NULL, $metadata = NULL) {
		...
		if ($objectdata != NULL) {
			return $this->sendRequest($objectname, "PUT", $objectdata, $acl, $contentType, $metadata, $bucketname);
		} else {
			return false;
		}
