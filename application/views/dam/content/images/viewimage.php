...
<div class="imageInformation clearfix"> 
    ...
    <?php if($next): if (strlen($tag_string)) $alt_tag_string = $tag_string . '/'; else $alt_tag_string = '';?>
		...
		<li class="next"><a class="thumb" href="<?=site_url('image/'.$alt_tag_string.$next->id.'/'.$location);?>" title="<?=$next->title;?>" style="background-image: url('<?=resizedImageURL('image_store/1500s/'.$next->previewname, 85, 85, true);?>')"> </a><a href="<?=site_url('image/'.$alt_tag_string.$next->id.'/'.$location);?>" title="<?=$next->title;?>">next &rarr;</a></li>
		<?php else:?>
		<li class="next"><a class="thumb" href="<?=site_url('dam_controllers/image/viewImage/'.$next->id.'/'.$location);?>" title="<?=$next->title;?>" style="background-image: url('<?=resizedImageURL('image_store/1500s/'.$next->previewname, 85, 85, true);?>')"></a><a href="<?=site_url('dam_controllers/image/viewImage/'.$next->id.'/'.$location);?>" title="<?=$next->title;?>">next &rarr;</a></li>
		<?php endif;?>
	<?php endif;?>
    
	<?php if($previous): if (strlen($tag_string)) $alt_tag_string = $tag_string . '/'; else $alt_tag_string = '';?>
		<?php if (strlen($tag_string)):?>
			<li class="prev"><a class="thumb" href="<?=site_url('image/'.$alt_tag_string.$previous->id.'/'.$location);?>" title="<?=$previous->title;?>"  style="background-image: url('<?=resizedImageURL('image_store/1500s/'.$previous->previewname, 85, 85, true);?>')"> </a><a href="<?=site_url('image/'.$alt_tag_string.$previous->id.'/'.$location);?>" title="<?=$previous->title;?>">&larr; previous</a></li>
		<?php else:?>
			<li class="prev"><a class="thumb" href="<?=site_url($package.'/image/viewImage/'.$previous->id.'/'.$location);?>" title="<?=$previous->title;?>"  style="background-image: url('<?=resizedImageURL('image_store/1500s/'.$previous->previewname, 85, 85, true);?>')"> </a><a href="<?=site_url($package.'/image/viewImage/'.$previous->id.'/'.$location);?>" title="<?=$previous->title;?>">&larr; previous</a></li>
		<?php endif;?>
	<?php endif;?>
